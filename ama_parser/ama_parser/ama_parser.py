...
def ama_parser(file_name, discard, blanks, data_labels, time_point, plot_title, sort_list,
               colors, skiprows=2, skipfooter=14, time_int = 0.25, output = "output_file.png",
    ...
    if plots == True:
        ...
        plt.bar(range(len(df2)), df2.average, align='center', yerr=df2.stderr, color=colors)
        plt.xticks(range(len(df2)), df2.protein)

        plt.ylabel("% of wildtype growth at " + str(time_point) + " hours")
